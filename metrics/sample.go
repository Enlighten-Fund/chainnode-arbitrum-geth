...
const (
	// SlidingTimeWindowCollisionBuffer allow this many duplicate ticks before
	...
	SlidingTimeWindowClearBuffer = time.Hour * SlidingTimeWindowCollisionBuffer
)
...
func (s *SlidingTimeWindowArraySample) trim() {
	...
	windowEnd := now + SlidingTimeWindowClearBuffer.Nanoseconds()
...
func (s *SlidingTimeWindowArraySample) Update(v int64) {
	...
	for {
		...
		if s.measurements.Put(newTick, v) {
			break
		}
	}
}
